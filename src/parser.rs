...
pub(crate) fn parse_field(s: &str) -> anyhow::Result<FieldKey> {
  ...
  let first = lex.next().ok_or_else(|| anyhow!("Empty field definition: '{}'", s))?;
  if let FieldToken::Word(word) = first {
    let field = word;

    match lex.next() {
      Some(FieldToken::LBracket) => {
        let et = lex.next().ok_or_else(|| anyhow!("Expected event type in '{}'", s))?;
        match et {
          FieldToken::Asterisk => {
      ...
      Some(FieldToken::Dot) => {
        ...
        let et = lex.next().ok_or_else(|| anyhow!("Expected event type after '.' in '{}'", s))?;
        if let FieldToken::Word(et) = et {
          ...
          if dot == Some(FieldToken::Dot) {
            ...
            if asterisk != Some(FieldToken::Asterisk) {
              return Err(anyhow!("Expected '*' after '.' in field definition: '{}'", s));
            }
          }
          Ok(FieldKey {
            field,
      ...
      _ => {
        Err(anyhow!("Invalid field definition: '{}'", s))
      }
    }
  } else {
